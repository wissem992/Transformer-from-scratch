...
def greedy_decode(model,source,source_mask,tokenizer_src,tokenizer_tgt,max_len,device):
  ...
  #precompute the encoder output and reuse for it for enveru token we get from decoder
  ...
  while True:
    if decoder_input.size(1) ==max_len:
      break
    #Build for the target (decoder input)
    ...
    #select the token with the max proba
...
def run_validation(model,validation_ds,tokenizer_src,tokenizer_tgt,max_len,device,print_msg,global_state,write,num_expamples=2):
  model.eval()
  count=0
...
def get_ds(config):
  ...
  # kepping for traininh and 10% for validation
...
def train_model(config):
  ...
  for epoch in range(initial_epoch,config['num_epochs']):
    ...
    for batch in batch_iterator:
      ...
      encoder_input=batch['encoder_input'].to(device) #(4,seq_len)
      decoder_input=batch['decoder_input'].to(device)#(4,seq_len)
      ...
      decoder_output=model.decode(encoder_output,encoder_mask,decoder_input,decoder_mask) #(batch,seq_len,model)
      ...
      global_step+=1
    run_validation(model,val_dataloader,tokenizer_src,tokenizer_tgt,config["seq_len"],device,lambda msg: batch_iterator.write(msg),global_step,write)
